...
#    for sentence in corpusRoot.getiterator(processElement):
    ...
    for sentence in input:
        infile.write("U" + str(idCount) + " " + sentence.replace("\n", " ").replace("\n", " ") + "\n")
        idCount += 1
    infile.close()
...
#    classPath += ":" + bannerPath + libPath + "commons-logging-1.1.1.jar"
    ...
    if oldVersion:
        ...
        for line in mentionfile:
            ...
            sentence = sDict[bannerId]
            map = getWhiteSpaceLessStringMap(sentence)
            offsets[0], offsets[1] = fixWhiteSpaceLessOffset(word, sentence, int(offsets[0]), int(offsets[1]), map)
            #offsets[0], offsets[1] = fixStrangeOffset(line.strip(), word, int(offsets[0]), int(offsets[1]), sentence.get("text"))
            entities = makeEntityElements(int(offsets[0]), int(offsets[1]), sentence, splitNewlines, elementName)
 ...
 #           for ent in entities:
 #               if ent.attrib['type']=='Protein':
 #                   entity_list.append(ent.attrib['text'])
            for ent in entities:
                entity_list.append(ent.attrib['text'])
                
       #         ent.set("id", sentence + ".e" + str(sentenceEntityCount[bannerId]))                
       #         sentence.append(ent)
                if not sentenceHasEntities[bannerId]:
                    sentencesWithEntities += 1
                    sentenceHasEntities[bannerId] = True
...
#       print >> sys.stderr, "Psyco not installed"
    ...
    if not sentence_list:
        input1 = ['Mutations in the tumor suppressor TP53 occur in almost all advanced ovarian cancers and in many advanced serous endometrial cancers', 'Mutations in TP53 can alter the function of the p53 protein, and some mutations result in a mutated protein with oncogenic activity']
    elif sentence_list:
        input1 = sentence_list
    print >> sys.stderr, "Running BANNER"
    entity_list = run(input=input1, output=options.output, elementName=options.elementName, 
